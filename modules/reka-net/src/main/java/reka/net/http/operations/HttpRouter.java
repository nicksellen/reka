...
public class HttpRouter implements RouterOperation {
	...
	public static final RouteKey OTHERWISE = RouteKey.named("otherwise");
	...
	private final boolean hasOtherwiseRoute;
	...
	public HttpRouter(Collection<Route> incoming, boolean hasElseRoute) {
		...
		this.hasOtherwiseRoute = hasElseRoute;
	...
	public void call(MutableData data, RouteCollector router) {
		...
		if (hasOtherwiseRoute && router.routed().isEmpty()) {
	...
	private static final Map<HttpMethod,Integer> methodOrderings = new HashMap<>();
	
	static {
		orderings.put(MountRoute.class, 10);
		orderings.put(StaticRoute.class, 20);
		orderings.put(RegexRoute.class, 30);
		methodOrderings.put(HttpMethod.GET, 10);
		methodOrderings.put(HttpMethod.PUT, 20);
		methodOrderings.put(HttpMethod.POST, 30);
		methodOrderings.put(HttpMethod.DELETE, 40);
	}
	...
	private static final Comparator<Route> routeComparator = new Comparator<HttpRouter.Route>() {
		...
		private int compareStaticRoutes(StaticRoute a, StaticRoute b) {
			if (a.method != b.method) {
				return compareMethods(a.method, b.method);
			} else {
		...
		private int compareRegexRoutes(RegexRoute a, RegexRoute b) {
			if (!a.method.name().equals(b.method.name())){
				return compareMethods(a.method, b.method);
		...
		private int compareMethods(HttpMethod a, HttpMethod b) {
			return Integer.compare(methodOrderings.getOrDefault(a, 100), methodOrderings.getOrDefault(b, 100));
		}
