...
public class HttpConfigurer extends ModuleConfigurer {
	...
	public HttpConfigurer(HttpServerManager server) {
		this.server = server;
	}
		...
		for (Function<ConfigurerProvider, OperationConfigurer> h : requestHandlers) {
			...
			module.trigger("on request", h, registration -> {
				...
				for (HostAndPort listen : listens) {
					...
					HttpSettings settings = httpSettings(listen, registration.applicationIdentity(), registration.applicationVersion());
	...
	private HttpSettings httpSettings(HostAndPort listen, String applicationIdentity, int applicationVersion) {
